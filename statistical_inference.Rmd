...
The expected value or mean of a exponential distribution is calculated by E[X]=1/λ. The standard deviation is equal to the mean and the variance is calculated by Var[X]=1/λ^2. In this case we assume that λ (=rate) is 0.2.
...
If we simulate a exponential distribution with a samplesize n=1000 and a rate λ=0.2, the histogram is looks as follows:
```{r }
hist(exponentialdistribution)
ed<-data_frame(val = exponentialdistribution)
str(ed)
n<-ggplot(data = ed, aes(x = val,  colour="red", fill = "red", alpha=0.1)) + theme(legend.position="none") + geom_density() +  geom_vline(xintercept = mean(ed$val), size = .75, color = "blue")
n
```
...
n<-ggplot(data = sam, aes(x = val,  colour="red", fill = "red", alpha=0.1)) + theme(legend.position="none") + geom_density() +  geom_vline(xintercept = mean(sam$val), size = .75, color = "blue")
n
```
